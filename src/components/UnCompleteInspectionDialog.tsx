...
interface UnCompleteInspectionDialogProps {
  open: boolean;
...
export function UnCompleteInspectionDialog({
  open,
  onOpenChange,
  mainInspection,
  connectedInspections,
  onConfirm,
}: UnCompleteInspectionDialogProps) {
  ...
  const handleConfirm = () => {
    ...
    const idsToUncomplete = [mainInspection.id, ...Array.from(selectedIds)];
    onConfirm(idsToUncomplete);
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Mark Inspection as Incomplete</DialogTitle>
          <DialogDescription>
            You are marking this inspection as incomplete:
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          {/* Main inspection being uncompleted */}
          ...
          {connectedInspections.length > 0 && (
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                Select additional inspections to mark as incomplete:
              </p>
        ...
        <DialogFooter>
          ...
          <Button variant="secondary" onClick={handleConfirm}>
            Mark {selectedIds.size > 0 ? `${selectedIds.size + 1} Inspections` : "Inspection"} as Incomplete
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
